...
describe('Get', () => {
  ...
  describe('comments', () => {
    ...
    it('should send correct json response if comments exist in database', async () => {
      ...
      jest.spyOn(commentService, 'getPostComment').mockResolvedValue([commentsData]);

      await Get.prototype.comments(req, res);
      expect(commentService.getPostComment).toHaveBeenCalledWith(
  ...
  describe('commentsNamesFromCache', () => {
    it('should send correct json response if data exist in redis', async () => {
      ...
      const res: Response = reactionMockResponse();
      jest.spyOn(CommentCache.prototype, 'getPostCommentNameFromCache').mockResolvedValue([commentNames]);

      await Get.prototype.commentsNamesFromCache(req, res);
      expect(CommentCache.prototype.getPostCommentNameFromCache).toHaveBeenCalledWith('6027f77087c9d9ccb1555268');
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
    ...
    it('should send correct json response if data exist in database', async () => {
      ...
      const res: Response = reactionMockResponse();
      jest.spyOn(CommentCache.prototype, 'getPostCommentNameFromCache').mockResolvedValue([]);
